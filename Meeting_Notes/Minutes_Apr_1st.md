# Meeting Minutes: Project Euro Valuation
#### (AI generated from transcript - Checked ✅)

**Date:** April 1, 2026
**Time:** 13:01 BST
**Attendees:** Will Barry, Rahul Shafia

## Key Discussion Points

### Market Outlook & Euro Drivers
* The team noted that public reports often lack long-term geopolitical predictability, restricting the two-year market outlook.
* Concerns were raised about private credit market stability, with William Barry characterizing a severe failure as "2008 territory".
* JP Morgan has reportedly started creating options for investors to bet against private credit.
* The Stone X report suggests a conservative Euro price decline to around 1.13 in Q2.
* The current Euro price is approximately 1.16.
* Geopolitical and macro factors are the dominant drivers of Euro value, primarily influenced by an unprecedented 55% jump in crude oil prices linked to Middle East conflicts.
* Implied volatility is notably high for currencies, sitting between 8% and 10%.
* An inverted Brent crude oil chart showed a positive correlation with the Euro to USD price, confirming high oil prices are detrimental to the European economy.

### Long-Term Strategy & Thesis
* The central thesis is to determine the Euro's appreciation or depreciation over a two-year period.
* The team will shift focus to long-term indicators, such as the volatility index, rather than relying solely on near-term data.
* William Barry highlighted the need to investigate long-term European and American policies impacting the EU's oil reliance, which is essential for defining risk scenarios where their thesis might become redundant.
* They discussed integrating both a short-term dip and a long-term long position to strengthen the overall trade recommendation.

### Extracurricular / Fund Updates
* The team discussed the Bristol Parent Fund, which trades with real money and adds a layer of seriousness to their work.
* The fund has achieved consistent 34% returns, largely due to its significant exposure to the strong South Korean stock market index.

---

## Action Items

### Rahul Shafia - complete sections 3 - 5
* Update Metrics: Add volatility index data to the report table for analysis.
* Supplement Report: Add further research consensus data to the report section.
* Research Policies: Look into long-term policies from Europe and America affecting EU oil reliance.
* Research Geopolitics: Examine geopolitical factors impacting the volatility index (VIX) to assess long-term Euro demand.

### William Barry - complete sections 6 & 8 (+ begin 1)
* Update Repository: Update repository files to reflect the 2-year Euro appreciation/depreciation thesis.
* Convert Report: Convert the current document into the required LaTeX report format.
* Fix Models: Fix technical issues within the vector order regression model and the correlation matrix.
* Write Verification: Write up the quantitative verification section using the standard template.
* Write Verification: Write up the quantitative verification section using the standard template.
* **Corrolation Matrix:** Apply the Block CPA/Hierarchical Factor Analysis to the corrolation matrix to improve signal to noise ratio.
* Research Geopolitics: Deliver geopolitical and macroeconomic factors that could impact EUR/USD (focus on terms of trade qualitative analysis; that is what is the economic "buffer" of the euro - will the shrinking car exports of germany be an issue for example?.

### Joint Actions
* Decide Probabilities: Look at risk pricing scenarios and use Teams chat to discuss and decide probabilities for bull and bear market outcomes.
